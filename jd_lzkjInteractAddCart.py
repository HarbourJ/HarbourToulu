...
jd_lzkjInteractUrl = os.environ.get("jd_lzkjInteractAddCartUrl") if os.environ.get("jd_lzkjInteractAddCartUrl") else ""
share_userId = os.environ.get("jd_lzkjInteractUserId") if os.environ.get("jd_lzkjInteractUserId") else ""
runNums = os.environ.get("jd_lzkjInteractAddRunNums") if os.environ.get("jd_lzkjInteractAddRunNums") else 12
...
runNums = int(runNums)
if runNums == 12:
    print('🤖本次加购默认跑前12个账号,设置自定义变量:export jd_lzkjInteractAddRunNums="需要运行加购的ck数量"')
else:
...
def addSkuAct(Token):
    url = "https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/api/task/addSku/activity"
    body = {}
    headers = {
        'Host': 'lzkj-isv.isvjcloud.com',
        'Accept': 'application/json, text/plain, */*',
...
def addSku(Token, taskId, skuId):
    url = "https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/api/task/addSku/toDo"
    body = {
        "taskId": taskId,
        "skuId": skuId,
    }
    ...
    try:
        ...
        if res['resp_code'] == 0:
            ...
            print(f"addSku Error: {res['resp_msg']}")
    except Exception as e:
        print(f"addSku Error: {e}")
...
if __name__ == '__main__':
    ...
    for cookie in cks[:runNums]:
        ...
        try:
            ...
            if "关注店铺" in joinCodeInfo['joinDes']:
                followShop(Token)
                print(f"关注店铺成功")
                time.sleep(0.2)
            ...
            skuInfo = addSkuAct(Token)
            # print(f"skuInfo: {skuInfo}")
            # print(f"addWares: {skuInfo['addWares']}")
            # print(f"prizeResultNum: {skuInfo['prizeResultNum']}")
            # print(f"followShopStatus: {skuInfo['addWares']['followShopStatus']}")
            finishNum = skuInfo['addWares']['finishNum']
            # print(f"finishNum: {finishNum}")
            completeCount = skuInfo['addWares']['completeCount']
            # print(f"completeCount: {completeCount}")
            oneClickPurchase = skuInfo['addWares']['oneClickPurchase']
            print(f"需要加购{finishNum},已加购{completeCount}")
            taskId = skuInfo['addWares']['taskId']
            skuInfoVO = skuInfo['addWares']['skuInfoVO']
            skuIds = [i['skuId'] for i in skuInfoVO if not i['status']]
            status = skuInfo['addWares']['status']
            if completeCount >= finishNum or status:
                print("已经完成过加购任务")
            else:
                needAddCount = finishNum - completeCount
                for x in range(needAddCount):
                    skuId = skuIds[x] if oneClickPurchase else ""
                    addSkuResult = addSku(Token, taskId, skuId)
                    # print(f"addSkuResult: {addSkuResult}")
                    if addSkuResult == 99:
                        if x == needAddCount - 1:
                            print(f"成功加购{needAddCount}个商品,获得💨💨💨")
                    else:
                        # 4 积分
                        prizeName = addSkuResult['prizeName']
                        prizeType = addSkuResult['prizeType']
                        print(f"🎁成功加购{needAddCount}个商品,获得{prizeName}")
