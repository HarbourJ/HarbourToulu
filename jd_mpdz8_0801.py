...
def inviteRelation(buyerNick):
    ...
    # print('inviteRelation', inviteRelation)
...
def completeMissionQa(buyerNick, missionType, problemId, answerIds):
    ...
    if completeMission['success']:
        ...
        if "错误" in remark:
            print(f"\t😭{remark[:5]}奖励一个💨")
        else:
            print(f"\t🎉{remark[:5]}获得10酿造币~")
    else:
...
def getDailyQA(buyerNick):
    ...
    # print('getDailyQA', response.text)
...
def getHotGoods(buyerNick, missionType):
    ...
    response = requests.request("POST", url, headers=headers, data=json.dumps(payload))
    getHotGoods = json.loads(response.text)
    # print('getHotGoods', missionType, response.text)
    if getHotGoods['success']:
    ...
    for ck in cks:
        ...
        if LA['success']:
            ...
            if drawAward['success']:
                if num == 1:
                    ...
                    for drawAward in drawAwardList:
                        numb += 1
                        print(f"奖品{numb}: {drawAward['awardName']} 剩余{drawAward['remainNum']}份 兑换需{drawAward['needNum']}坛白酒")
            time.sleep(1)
            ...
            for i in data[:-1]:
                # print(i)
                if i['isComplete']:
                    print(f"{i['missionName']} 已完成")
                else:
                    print(f"现在去做{i['missionName']}任务")
                    # 购买商品(跳过)
                    if i['type'] == 'payTrade':
                        print("\t😆先v船长999")
                        time.sleep(0.5)
