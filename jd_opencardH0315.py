...
"""
File: jd_opencardH0315.py(3.15-3.19 酒水品牌会员日)
Author: HarbourJ
Date: 2023/3/15 10:00
TG: https://t.me/HarbourToulu
cron: 0 0 */3 15-19 3 *
new Env('3.15-3.19 酒水品牌会员日');
ActivityEntry: https://lzdz1-isv.isvjcloud.com/m/1000004718/dz0b06e6ee4dd59deecf0dd6aa6674
https://lzdz1-isv.isvjcloud.com/m/1000004718/741673/dz0b06e6ee4dd59deecf0dd6aa6674/?shareUuid=ced0237d4fb54bb2bb199c59557f3273&adsource=null
Description: 开卡50 邀请20
            并发变量: export jd_opencardH0315_uuid="你的助力码"
"""
...
inviterUuid = os.environ.get("jd_opencardH0315_uuid") if os.environ.get("jd_opencardH0315_uuid") else ""

venderId = "1000004718"
activityId = "dz0b06e6ee4dd59deecf0dd6aa6674"
activity_url = f"https://lzdz1-isv.isvjcloud.com/m/1000004718/741673/{activityId}/?shareUuid={inviterUuid}&adsource=null"
print(f"【🛳活动入口】https://lzdz1-isv.isvjcloud.com/m/1000004718/741673/{activityId}")
...
def activityContent(pin, pinImg, nickname):
    url = "https://lzdz1-isv.isvjcloud.com/dingzhi/march/drink/activityContent"
    try:
...
def checkOpenCard(pin, actorUuid, shareUuid):
    url = "https://lzdz1-isv.isvjcloud.com/dingzhi/march/drink/checkOpenCard"
    ...
    response = requests.request("POST", url, headers=headers, data=payload)
    # print(f"checkOpenCard:{response.text}")
    refresh_cookies(response)
    res = response.json()
    if res['result']:
        return res['data']
...
def saveTask(actorUuid, shareUuid, pin, taskType, taskValue):
    url = "https://lzdz1-isv.isvjcloud.com/dingzhi/march/drink/saveTask"
    ...
    for cookie in cks[:]:
        ...
        try:
            ...
            if getPin is not None:
                ...
                drawContent(pin)
                time.sleep(0.2)
                initOC = checkOpenCard(pin, actorUuid, shareUuid)
                allOpenCard = initOC['allOpenCard']
                isAssist = initOC['canReceive']
                assistStatus = initOC['assistStatus']
                openInfo = initOC['openList']
                if allOpenCard:
                    print("已完成全部开卡任务")
                    print(f"助力状态-->{assistStatus}")
                    if assistStatus == 0:
                        print("无法助力自己")
                    elif assistStatus == 2:
                        print("已经助力过好友")
        ...
        if num == 1:
            ...
            shareUuid = actorUuid
            activityUrl = f"https://lzdz1-isv.isvjcloud.com/m/1000004718/8487146/{activityId}/?shareUuid={shareUuid}&adsource=null"
